...
static void _nrom_mem_write(void *mem, uint16_t addr, uint8_t data)
{
    ...
    switch (addr)
    {
        case 0x2000: ppu_write(&nrom->ppu, PPUIO_CTRL, data);
        case 0x2001: ppu_write(&nrom->ppu, PPUIO_MASK, data);
        case 0x2003: ppu_write(&nrom->ppu, PPUIO_OAMADDR, data);
        case 0x2004: ppu_write(&nrom->ppu, PPUIO_OAMDATA, data);
        case 0x2005: ppu_write(&nrom->ppu, PPUIO_SCROLL, data);
        case 0x2006: ppu_write(&nrom->ppu, PPUIO_ADDR, data);
        case 0x2007: ppu_write(&nrom->ppu, PPUIO_DATA, data);
        case 0x4014: ppu_write(&nrom->ppu, PPUIO_OAMDMA, data);
        default:
...
uint8_t nrom_load(uint8_t *ines, struct nrom *out)
{
    ...
    *out = (struct nrom){ 0 };
    ...
    uint32_t bytes = out->prgsize*(1<<14) + out->chrsize*(1<<13);
    ...
    memcpy(out->memory+0x8000, ines+16, bytes);
...
void nrom_frame(struct nrom *nrom)
{
