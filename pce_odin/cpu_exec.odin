...
cpu_exec_instr :: proc(cpu: ^CPU) {
  ...
  case .ADC:
    ...
    a := cpu_read_a(cpu)
    if cpu.p.dec {
    	res, car := bcd_add(val, a, cpu.p.car)
     
     	cpu.p.neg = (res&0x80) > 0
     	cpu.p.car = car
      // TODO: this is probably wrong
      cpu.p.ovf = (~(a ~ val) & (a ~ res) & 0x80) > 0
      cpu.p.zer = res == 0
      cpu_cycle(cpu) // here?
      
      cpu_write_a(cpu, res)
    } else {
      calc := cast(u16)a + cast(u16)val + cast(u16)cpu.p.car
      result := cast(u8)(calc&0xFF)

      cpu.p.neg = (result & 0x80) > 0
      cpu.p.car = calc > 0xFF
      // learned from mesen ^^ bitwise (sign(a) == sign(val) && sign(a) != sign(result))
      cpu.p.ovf = (~(a ~ val) & (a ~ result) & 0x80) > 0
      cpu.p.zer = result == 0

      cpu_write_a(cpu, result)
    }
  case .SBC:
    val := adr_mode_read_u8(cpu, adr)
    a := cpu_read_a(cpu)
    if cpu.p.dec {
   		res, car := bcd_sub(val, a, cpu.p.car)
